...
- 태그 섹션별 "+추가" 타일 → `openAddModal(tag)` → 모달(뼈대)

## 이식 안 됨 / 축소해서 이식
- 항목 추가 모달은 **영상 링크 저장까지만** 동작 — `clip_start`/`clip_end`는 항상 `null`로 저장 (레거시의 클립 마킹 플레이어 없음)
- 이미지 유형 자체가 모달에 없음 (Cropper.js 미도입, 유형 토글 버튼도 없음)
- 개별 항목 삭제(`deleteItem` 상당 기능) 없음 — 맵 삭제만 가능
