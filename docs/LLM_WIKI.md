...
```text
index.html
  ├─ Supabase/Cropper CDN 로드
  ├─ styles.css 로드
  ├─ 화면 DOM과 인라인 onclick 제공
  └─ body 끝에서 app.js 실행
...
- 빌드 도구와 번들러가 없는 정적 사이트다.
- `app.js`는 ES Module이 아닌 classic 전역 스크립트다.
- `index.html`의 인라인 `onclick`이 전역 함수에 의존한다.
- `<head>`의 짧은 테마 선적용 스크립트는 초기 테마 깜빡임을 막기 위해 HTML에 남아 있다.
- Vercel은 HTML/CSS/JS를 한 프로젝트의 정적 파일로 함께 배포한다.

## 작업별 코드 지도

| 작업 | 먼저 볼 함수/영역 | 함께 볼 문서 |
|---|---|---|
| 초기 데이터와 공개 범위 | `loadAll()`, `publicItems()`, `refreshPublicDataIfStale()` | `architecture/database-flow.md`, `DATABASE.md` |
| 홈 대시보드 | `renderHome()`, `renderMyItems()`, `openHomeAdd()`, `recentItemsKey()` | `PROJECT_STRUCTURE.md`, `TODO.md` |
| 전체 맵과 검색 | `renderMapGrid()`, `renderGlobalTitleSearch()`, `openMap()` | `architecture/search-flow.md` |
| 상세 카드와 즐겨찾기 | `renderCards()`, `favoriteButton()`, `toggleFavorite()` | `architecture/search-flow.md`, `tests/favorites.test.js` |
| 인증과 관리자 판별 | `initAuth()`, `renderAuthArea()`, `discordLogin()` | `architecture/auth-flow.md`, `DATABASE.md` |
| Master 대시보드 | `openMaster()`, `switchMasterTab()`, `loadMasterStats()`와 `renderMaster*` 함수 | `architecture/admin-flow.md` |
| 승인 흐름 | `renderMasterApprovals()`, `reviewItem()`, `renderMyItems()`, `hideOwnItem()` | `architecture/admin-flow.md`, `DATABASE.md` |
...
| 댓글 | `renderCommentsSection()`, `loadComments()`, `submitComment()`, `deleteComment()` | `DATABASE.md`, `architecture/admin-flow.md` |
...
| 테마·폰트·레이아웃 | `styles.css`, `renderThemeToggle()`, `toggleTheme()` | `PROJECT_STRUCTURE.md`, `DECISIONS.md` |
...
- 공개 화면은 반드시 `publicItems()`를 거쳐 `published` 항목만 사용한다.
- 일반 사용자 등록은 `pending`, 관리자 등록은 `published`다.
- 작성자 이름과 아바타 등 신뢰 경계는 클라이언트만 보지 말고 `DATABASE.md`의 트리거와 RLS를 함께 확인한다.
- Supabase 호출은 대부분 예외가 아니라 `{ data, error }`를 반환한다.

## 변경 시 같이 확인할 것

- 전역 함수명을 바꾸면 `index.html`과 템플릿 문자열의 인라인 이벤트 참조를 함께 찾는다.
- 전역 상태를 바꾸면 홈·전체 맵·상세·Master 중 다시 렌더해야 하는 화면을 확인한다.
...
자동 생성 위키는 `.code-review-graph/wiki/`에 저장되며 캐시 성격이라 git에는 포함하지 않는다. 현재 `app.js`가 하나의 전역 파일이어서 커뮤니티 자동 분류는 테스트 코드 중심으로만 생성된다. 따라서 자동 위키를 프로젝트 전체 구조의 기준으로 간주하지 말고, 이 문서와 `AI_CONTEXT.md`를 기준으로 사용한다.
...
3. 이 문서와 `PROJECT_STRUCTURE.md`, `architecture/*.md`
