...
## 2026-08-05 — 제작자 문의 기능 (신규)

- 신규 테이블 `inquiries`(카테고리/내용/읽음 상태/작성자 스냅샷) + RLS(등록은 본인만, 조회·읽음처리는 관리자만) + 전용 작성자 스냅샷 트리거 `set_inquiry_contributor()` 생성(`items`의 `set_item_contributor()`는 `submitted_at` 컬럼 의존으로 재사용 불가해 별도 작성)
- `is_read` 외 컬럼은 관리자도 UPDATE로 바꿀 수 없도록 컬럼 단위 권한(`grant update (is_read)`)으로 추가 제한
- 전역 플로팅 버튼(FAB) + 문의 작성 모달(카테고리 3종 + 내용) 추가. 비로그인 클릭 시 `openHomeAdd()`와 동일하게 confirm 없이 바로 Discord 로그인. Master 화면에서는 `body:has(#viewMaster.active)` CSS로 자동 숨김
- Master 사이드바 6번째 탭 "문의함" 추가(카테고리·읽음 필터, 읽음 토글). 답장 기능 없음(일방향)
- `tests/browser-history.test.js`의 Master 뒤로가기 버튼 개수 기대값을 5→6으로 갱신(6번째 탭도 동일 패턴)

## 2026-08-05 — 컨텐츠 추가 모달: 안내 문구 개선 + 제목 서체

- 붙여넣기 박스 힌트를 "Ctrl+V"→"유튜브 링크 또는 이미지 복사 후 눌러주세요 (자동으로 가져와요)"로, 파일 선택 박스 힌트를 "이미지만"→"이미지 파일만 업로드할 수 있어요"로 교체(디바이스 분기 없이 동일 문구, "모바일 길게 눌러 붙여넣기" 초안 문구는 커밋 전 리뷰에서 버튼 요소엔 적용 불가함이 드러나 실제 자동 읽기 동작에 맞게 수정)
- 모달 제목(`.modal-head h3`, 추가·수정 모달 공유)에 히어로와 동일한 기존 Paperlogy 800 적용(신규 폰트 굵기 로드 없음), 박스 라벨·힌트는 기존처럼 Pretendard 유지
- 데스크톱 좌우 배치는 기존 CSS(`.paste-choices` 기본 `flex-direction:row`, 600px 이하에서만 column)로 이미 구현돼 있어 레이아웃 코드 변경 없음(확인만 진행)

## 2026-08-05 — 홈 히어로 재설계 + 하단 3개 섹션 빈 상태 카드화

- 히어로 태그 "TACTICAL ARCHIVE"→"ARCHIVE", 헤드라인·부제 문구를 확정 카피로 교체(서체는 기존 그대로 — 태그 Rajdhani/Pretendard, 헤드라인 Paperlogy 800 유지, 700 신규 로드 없음)
- 배경 장식 원형 그래픽(`.home-hero::after`) 제거, 그 자리에 통계 카드 2개(위폭 클립 수/팁 수, 헤더 카운터와 동일 집계 재사용) + "전체 보기" 버튼(구 "전체 맵 보기", 목적지 동일)을 한 줄 가로 배치
- 하단 3개 섹션(내가 추가한 컨텐츠/내 즐겨찾기/최근 본 컨텐츠) 빈 상태를 콘텐츠 카드와 같은 240px 고정 폭 + 36px 원형 회색 아이콘 배지로 통일, 버튼 제거
- "내가 추가한 컨텐츠"/"내 즐겨찾기" 빈 상태는 박스 전체 클릭으로 기존 버튼 목적지(컨텐츠 추가 모달/전체 맵)로 이동, "최근 본 컨텐츠"는 기존에 있던 이동 버튼을 제거하고 클릭 무반응으로 변경
- `emptyStateHtml()`에 선택 파라미터 `extraClass`/`boxOnclick` 추가(기존 4개 호출부는 동작 변화 없음)

## 2026-08-05 — UI 개선: 카드 배지 위계 / 빈 상태 / 아이콘 시스템 통일

- 유형 배지(영상/이미지)를 브랜드 핑크(`--edit-accent`) 실색 배지로 통일(기존 red/blue 구분 색상 제거), 좌상단 유지
- 진영/공동 배지(`teamBadge()`)를 아웃라인 스타일로 변경하고 우상단(즐겨찾기 별 버튼 아래, `top:40px`)으로 재배치. 쇼츠 배지는 좌측으로 옮겨 유형 배지와 세로로 쌓음(즐겨찾기·쇼츠·진영 배지 3요소 충돌 방지)
- 공용 함수 `emptyStateHtml()` 신설, 홈 3곳(즐겨찾기/최근 본 컨텐츠/내가 추가한 컨텐츠 없음)·전체·상세 검색 결과 없음·Master 승인 대기 없음 6곳에 아이콘+헤드라인+설명(선택)+바로가기 버튼(선택) 구조로 통일. 기존 `.home-empty` CSS 삭제(사용처 없어짐)
- `@tabler/icons-webfont@3.31.0` CDN 도입(jsdelivr, cropperjs와 동일 패턴), 사이트 전체 이모지·유니코드 기호 아이콘(📋🖼👑☆★✕✎⚙🗑🔒💬✅🗺📊🎯💡🔍⏸▶🔊🔇↺＋)을 `<i class="ti ti-이름">` 아웃라인 아이콘으로 교체
- `textContent`로 전체 텍스트를 갈아끼우던 동적 토글(오버레이 재생/일시정지·음소거, 클립 재생 아이콘)을 `innerHTML`로 `<i>` 태그를 교체하는 방식으로 변경
- 뒤로가기 화살표(`←`), 클립 `−10`/`+10`, CSS로 그린 `.playicon`, `confirm()` 경고 문구의 이모지는 교체 대상에서 제외(명시적 예외)
- Node 테스트(`favorites.test.js`)를 새 아이콘 마크업(`ti-star` + `class="card-fav on"` 유무로 활성 판별)에 맞게 갱신
...
- `window.addEventListener('popstate', ...)`로 뒤로가기 시 같은 화면 진입 함수를 재사용해 복원, 복원 중에는 `restoringFromHistory` 플래그로 중복 push 방지
- 맵 상세는 mapId/mapName/team까지 상태에 포함해 정확히 복원. 팀 전환은 별도 항목이 아니라 `replaceState()`로 현재 항목만 갱신
- Master 대시보드는 진입/이탈만 한 단계로 취급(내부 탭 전환은 히스토리 항목 아님)
- 오버레이 "닫기 ✕", 맵 상세 "← 전체 맵으로", Master 각 탭의 "← 이전 화면으로"(기존 "← 일반 화면으로"/"← 홈으로"에서 라벨 변경)를 `history.back()`으로 재배선해 버튼 클릭과 물리적 뒤로가기가 항상 같은 히스토리 항목 1개를 소비하게 함
- 모달은 히스토리에 없음 — popstate 시 열려 있는 모달을 `requestCloseModal()`로 먼저 정리한 뒤 화면 복원
- 세션 만료로 Master에서 강제 이탈되는 기존 안전장치는 새 항목을 쌓지 않고 현재 항목을 grid로 교체하도록 수정
- 딥링크(URL로 특정 화면 열기)는 이번 범위 아님
...
- 승인 표에 태그 열을 표시하고 전체 대기열 없음과 조건별 결과 없음 문구를 구분
- 기존 로드 데이터와 Master 필터 스타일만 재사용해 추가 DB 조회·스키마 변경 없이 구현
- Node 회귀 테스트와 Chromium 조합 필터·390px 모바일 레이아웃 검증 완료

## 2026-08-04 — 그룹 F-6 맵 지명 관리자 전용 + Master 휴지통 복구

- 일반 사용자 컨텐츠 추가 태그를 `위폭`/`팁`으로 제한하고 모달·등록·임시저장 복원 가드와 Supabase INSERT/UPDATE RLS를 함께 적용해 직접 API 우회 차단
- `items_tag_check`, `trashed_from_status`, 이전 상태 보존 트리거를 추가해 휴지통 이동 후 `published`/`pending`/`rejected` 원래 상태로만 복구되도록 강제
- Master 항목 관리에 `활성 항목/휴지통` 전환, 이전 상태·이동 시각·복구 UI를 추가하고 기존 영구 삭제 버튼을 복구 가능한 휴지통 이동으로 교체
- 일반 사용자/관리자 RLS 롤백 테스트, 이전 상태 위조 방어, Chromium 데스크톱·390px UI 검증 완료. 영구 삭제 UI는 제외

## 2026-08-04 — 클립 중앙 조작부 + 맵 지명 세로 카드

- 클립 편집 플레이어 아래에 현재 시간·`−10`·재생/일시정지·`+10`·전체 길이를 중앙 조작군으로 추가하고 기존 구간 감시 타이머로 시간 표시와 반복 재생을 함께 처리
- 10초 이동은 전체 영상 범위에서 이동한 뒤 일시정지 프레임을 유지하고, 선택 구간 밖에서 재생하면 시작 지점으로 복귀해 구간 재생
- 맵 지명 카드를 4:5 세로 비율의 이미지 전용 카드로 변경하고 제목·작성자 메타데이터를 맵 상세·홈·전체 검색에서 제거
- 실제 맵 지명 이미지가 세로 프레임에서 잘리지 않도록 해당 카드만 `object-fit: contain` 적용
- Node 회귀 테스트와 Chromium에서 PC 조작·실데이터 카드 렌더 확인
...
- 홈·Master 6탭·승인 흐름·5분 재조회·폰트 역할을 구조/인증/검색/관리자/데이터 흐름 문서에 반영
- `DATABASE.md`에서 `items` 생명주기 컬럼이 `maps` 표에 잘못 들어간 문서 오류와 오래된 제목·설명 제한값(14/35 → 17/41), 역할별 RLS 설명을 수정

## 2026-08-03 — 한글 타이포그래피 역할 정리 (`d8bdf16`)

- 본문·일반 UI를 Pretendard로 통일하고 댓글 영역에 조선굴림체 적용
- Paperlogy 제목과 Rajdhani/JetBrains Mono의 기존 영문·숫자 역할은 유지하면서 한글 fallback만 Pretendard로 교체해 `GulimChe` 렌더링 제거
- 사용하지 않던 Gothic A1·Inter import 제거, 실제 platform font와 CDN 응답 검증

## 2026-08-03 — Notion 개발 기록 자동 갱신 규칙 (`7afb28f`)

- 기능 구현·버그 수정·DB/RLS·배포처럼 개발 이력으로 남길 실질 작업 완료 시 Notion `개발 공부 → 개발 기록 → 서든 아카이브`를 중복 없이 갱신하도록 `AGENTS.md`에 운영 규칙 추가
- 단순 조회·설계·미완료 작업과 비밀값·인증 토큰은 자동 기록 대상에서 제외

## 2026-08-03 — 홈 대시보드와 사용자 컨텐츠 승인 흐름 (`c78be0b`)

- 비로그인도 홈을 첫 화면으로 사용하고 승인된 컨텐츠의 전체 맵으로 가는 CTA, 로그인 전용 즐겨찾기·컨텐츠 추가 안내, 최근 본 컨텐츠 구현
- `items`에 작성자·`pending/published/rejected/trashed` 생명주기와 검토 메타데이터를 추가하고 역할별 RLS, Discord 작성자 이름·아바타 강제 저장 적용
- 일반 사용자 등록·수정·승인 전 숨김, 관리자 승인·반려, 승인 후 관리자 전용 삭제와 5분/탭 복귀 공개 데이터 갱신 구현

## 2026-08-03 — Master 댓글 모아보기 (`d7e9840`)

- Master 댓글 탭에서 전체 댓글 최신순 조회, 맵 필터·본문/작성자/항목 검색, 항목 미리보기와 삭제 구현
- 관리자 미리보기가 클릭수에 포함되지 않도록 분리하고 오버레이 댓글 삭제 시 Master 캐시도 즉시 갱신

## 2026-08-03 — 컨텐츠 추가 개선 (`6d361da`)

- Master "영상 추가"를 "컨텐츠 추가"로 개칭하고 클립 슬라이더 트랙·손잡이 확대
- `sa-content-drafts` 기반 다중 임시저장, 이어서 작성·삭제·등록 완료 시 정리 구현

## 2026-08-03 — 이미지 오버레이 확대·이동 (`e41ca3f`)

- 이미지 항목에 휠/핀치 확대, 마우스·터치 드래그 이동, 리셋·더블클릭·더블탭 복귀 구현
