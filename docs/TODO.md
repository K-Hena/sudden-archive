...
- **그룹 D-2: 관리자 Master 대시보드** — 1~4단계(Master 버튼·페이지 셸·통계 탭 / 영상 추가 / 항목 관리 / 맵 관리 + 편집모드 제거) 완료. 그룹 J(댓글 기능) 완료로 5단계(댓글 모아보기 탭)는 이제 착수 가능
...
- 타이틀('서든 아카이브') 클릭 시 홈 대시보드로 이동 — 로그인 유저는 즐겨찾기 영상만 모아보는 화면 포함, 내용 구상 필요
- 홈 대시보드 구성(타이틀 클릭 시 이동, 위 항목과 연결) — 즐겨찾기 뷰, 본인이 추가한 컨텐츠 모음(승인 여부 무관하게 전부 표시) 포함 검토
- Master "컨텐츠 추가" 기능을 홈 대시보드로 이전 검토 — 현재 Master 5탭 중 하나로 구현돼 있으나(D-2 2단계), 홈 대시보드 완성 시 그쪽으로 옮기는 방향 고려. 유저 임베드+승인 기능과 연계될 가능성 있음 → **확정**: 홈 대시보드가 실제로 설계·구현되는 시점에 그쪽으로 이전 진행
...
- 유저 영상·이미지 임베드 기능 — 홈 대시보드에서만 노출, 등록 시 본인만 모아보기 가능, 관리자 승인 후 다른 사용자에게도 공개(승인 워크플로우 필요, 신규 스키마)
- Master에 "컨텐츠 승인" 탭 신설 — 위 유저 영상·이미지 임베드 기능(승인 워크플로우)과 연결되는 관리 화면. 현재 사이드바 5탭에서 6탭으로 확장 필요
...
- D-2c. 댓글 모아보기 탭 (그룹 J 댓글 기능 완료, 이제 착수 가능)
