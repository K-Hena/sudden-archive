...
- **그룹 F: 홈 대시보드 + 사용자 컨텐츠 등록·승인**
  ...
  - [ ] **F-4 승인 탭 필터·운영 보완** — 맵·태그 필터와 관리자 휴지통 이동/복구가 실제 운영에 필요한지 사용 후 결정하고 필요 시 구현
  - [ ] **F-5 진입점 정리** — 홈 이전과 승인 흐름 검증 후 Master의 기존 `컨텐츠 추가` 탭 제거, 중복 임시저장 UI 정리, 구 Admin 사이트 기능 격차 재확인
...
- '맵 지명' 타입 카드 레이아웃을 세로로 길게 변경 — 다른 카드(위폭·팁)와 다른 비율 적용 검토, 이유/디자인 방향은 추후 설계 시 논의
- 클립 구간 설정 UI 수정 (2건)
  - 버그: 구간의 끝(또는 시작) 지점을 조정해도 미리보기가 항상 처음 재생 화면으로 돌아감. 클립 시작/끝 손잡이를 조작할 때, 일시정지 상태에서 그 손잡이에 해당하는 시점의 실제 프레임이 미리보기에 표시되도록 수정 필요
  - 개선: 디스코드 클립 편집 UI처럼 재생/일시정지 버튼 + "10초 전"/"10초 후" 이동 버튼을 클립 설정 화면에 추가하는 것 검토. 이미 클립 편집 화면에 떠 있는 유튜브 플레이어(`clipYtPlayer`)를 그대로 재사용해, "재생" 버튼은 `clipStart` 지점으로 이동 후 재생을 시작하고 `clipEnd`에 도달하면 정지(또는 `clipStart`로 복귀)하는 방식으로 구현 — 일반 재생 오버레이가 이미 하고 있는 구간 반복 로직과 사실상 같은 원리. "10초 전/후" 버튼은 현재 위치에서 ±10초 이동
