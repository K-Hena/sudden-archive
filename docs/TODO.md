...
- **그룹 A: 맵 상세뷰 진영 뷰 확장(TOTAL/RED/BLUE/FAVORITE) + 진영 없음(공통) 항목 지원** — `items.team`에 `'none'`(공통) 값 추가(CHECK 제약 교체), 팀 토글 4개 버튼, TOTAL/FAVORITE 전용 진영 텍스트 배지, 항목 추가 모달의 `teamWrap`(RED/BLUE/공통 선택) 도입. 세부 결정은 `docs/DECISIONS.md` 참고

# 예정 (AI_CONTEXT.md 기준)

- 구 Admin 사이트(`sudden-archive-admin.vercel.app`) 정리 — 편집모드가 CRUD 전체를 대체한 뒤 진행
- **그룹 B: 항목 추가 모달 개선** — 진영 선택 필드 추가, 구간선택/크롭 후 미리보기, 붙여넣기 버튼 UI 개선
