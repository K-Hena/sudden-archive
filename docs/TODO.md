...
- ~~카드 그리드 고정 브레이크포인트 전환(PC 5열/태블릿 3열/모바일 1열) + 글자수 제한 재산정~~ (완료) — 유동 그리드(`auto-fill minmax`)를 고정 열 구조로 전환, PC 브레이크포인트 1408px 실측 확정(패딩 64px+gap 64px+카드 256px×5). `renderGlobalTitleSearch()`가 지도 선택용 `.map-grid`를 잘못 상속받던 기존 버그도 함께 발견해 수정. 새 PC 카드 폭(230px) 기준 `maxlength`를 제목 17자·설명 41자로 재산정. 세부 결정은 `docs/DECISIONS.md` 참고
