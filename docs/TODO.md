...
- **"맵 지명" 태그 이미지 업로드** — 파일 선택 또는 Ctrl+V 붙여넣기로 이미지를 Storage `items/{timestamp}.jpg`에 업로드(크롭 없이 원본 그대로). "맵 지명" 추가 모달에서 유튜브 링크 입력이 함께 노출되던 버그도 함께 수정
- **편집모드 항목(카드) 개별 삭제** — 레거시 Admin의 `deleteItem()` 이식, 카드 호버 시 ✕ 아이콘으로 confirm 후 삭제

# 진행 중 (뼈대만 있고 미완성)

- **항목 추가 모달**: "위폭"/"팁" 태그는 여전히 영상(vid)만 지원. 이미지 업로드는 "맵 지명" 태그에만 적용됨 (위폭/팁에 이미지 업로드를 추가하는 것은 별도 작업)
- **항목 추가 모달**: 클립 구간(`clip_start`/`clip_end`) 마킹 UI 없음 — 현재 항상 `null`로 저장. 레거시 Admin에는 유튜브 플레이어로 시작/끝 지점을 찍는 UI가 있음
- **이미지 크롭**: Cropper.js 미도입 — 현재는 원본 이미지를 그대로 업로드 (`docs/DECISIONS.md` 참고, 도입 여부는 별도 결정 필요)
