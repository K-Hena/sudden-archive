...
- **홈 히어로 재설계 + 하단 3개 섹션 빈 상태 카드화** — 히어로 문구를 개인적/친근한 톤으로 교체하고 배경 장식 원형 그래픽을 헤더와 동일 집계를 재사용하는 통계 카드 2개로 대체. 하단 3개 섹션(내가 추가한 컨텐츠/내 즐겨찾기/최근 본 컨텐츠) 빈 상태를 콘텐츠 카드와 같은 240px 폭 + 원형 회색 배지로 통일하고 버튼 대신 박스 전체 클릭으로 이동(최근 본 컨텐츠는 무반응). `emptyStateHtml()`에 선택 파라미터 `extraClass`/`boxOnclick` 추가(기존 4개 호출부 영향 없음). 세부 결정은 `docs/DECISIONS.md` 참고
- **제작자 문의(쪽지) 기능 + Master 문의함 탭(D-2d)** — 로그인 필수·일방향(답장 없음) 문의 기능. 신규 테이블 `inquiries`(카테고리·내용·읽음 상태, `items`와 별도의 작성자 스냅샷 트리거 `set_inquiry_contributor()`) + RLS(등록은 본인만, 조회·읽음처리는 관리자만이며 `is_read` 외 컬럼은 컬럼 단위 권한으로 UPDATE 자체를 차단) 생성. 전역 플로팅 버튼(FAB, `.view` 4개 밖 body 레벨 고정 요소, `body:has(#viewMaster.active)` CSS로 Master에서만 숨김) + 문의 작성 모달(기존 모달 셸·성공 메시지 패턴 재사용, 새 CSS 없음). Master 사이드바 6번째 탭 "문의함"(카테고리·읽음 필터, 읽음 토글). 세부 결정은 `docs/DECISIONS.md` 참고
...
- **그룹 F: 홈 대시보드 + 사용자 컨텐츠 등록·승인**
  ...
  - [x] **F-3 후속: 맵 자유 텍스트 입력 → 드롭다운 + 태그 타일** — `openHomeAdd()`의 `prompt()` 두 번(맵 이름/태그)을 제거하고, "컨텐츠 추가" 클릭 시 모달이 바로 붙여넣기 단계로 열리도록 변경. 붙여넣기 완료 직후 새 `target` 단계(맵 드롭다운 + F-6a 권한 로직을 재사용한 태그 타일)를 추가해 오타·중복 맵 이름 생성 위험을 없앴다. 임시저장(맵·태그 미선택 상태 포함)도 지원. 세부 결정은 `docs/DECISIONS.md` 참고
...
- [x] 브라우저 뒤로가기(History API) 지원 — 홈/전체 맵/맵 상세/항목 오버레이/Master 5개 화면을 각각 히스토리 항목으로 취급. URL은 바꾸지 않고 `pushState`로 상태 객체만 쌓는 방식. 모달·Master 탭 전환은 범위 밖. `docs/ARCHITECTURE.md`의 "브라우저 히스토리(뒤로가기)" 참고

# 아이디어 (AI_CONTEXT.md "향후 개발 예정", 구체화 전)

- 검색 기능 고도화 — 1·2·3단계 완료(필드 확장·초성 검색·자동완성). `docs/ARCHITECTURE.md`의 "탐색/검색 흐름" 참고
- 검색 빈 상태 추천 — 검색어 입력 전 인기/최근 검색 추천(4단계 후보, 3단계 자동완성 범위에서 제외됨). 구체화 전
- 딥링크 지원 — 특정 화면(맵 상세, 항목 오버레이 등)을 URL로 바로 열기. 브라우저 뒤로가기 지원(History API, 상태 객체만 push)에서 명시적으로 범위 제외됨. 구체화 전
- `inquiries` 삭제(DELETE) 기능 — 제작자 문의 기능 지시서가 명시적으로 이번 범위에서 제외. 현재 RLS에 DELETE 정책이 없어 클라이언트로는 삭제 불가(관리자 권한 SQL로만 가능). 구체화 전
...
### 신규 스키마 필요
- ~~제작자 문의(쪽지) 기능 (로그인 필수, 익명 불가)~~ (완료 — "제작자 문의(쪽지) 기능 + Master 문의함 탭(D-2d)"로 구현, `docs/DECISIONS.md` 참고)
...
- ~~D-2d. 문의함 탭~~ (완료 — 제작자 문의 기능과 함께 구현, `docs/DECISIONS.md` 참고)
