...
## code-review-graph 자동 갱신 훅 누락

- **발견 시점**: 2026-07-27, 그룹 D-2 2단계 커밋 전후 점검 중
- **내용**: code-review-graph 자동 갱신 훅이 최소 1회(stage2 커밋) 그래프 갱신을 놓친 사례 있음. 수동 `build_or_update_graph_tool` 호출로 즉시 해결 가능. 재발 시 훅 설정 재점검 필요
- **위험도**: 낮음 — 그래프가 코드리뷰 보조용 캐시일 뿐 앱 동작에 영향 없음
- **상태**: 미해결(수동 우회로 해소)
