...
그룹 A~E와 그룹 D-2 1~5단계는 완료됐다. 그룹 F도 홈 대시보드, 사용자 등록 생명주기, Master 승인 기본 흐름과 F-5 진입점 정리까지 모두 완료됐다. 최신 세부 상태는 `docs/TODO.md`를 기준으로 한다.
...
**그룹 F(홈 대시보드 + 사용자 컨텐츠 등록·승인)는 F-5까지 구현 완료됨:**
...
- 남은 것은 운영 후 필요 여부를 결정할 승인 필터·휴지통뿐이다
