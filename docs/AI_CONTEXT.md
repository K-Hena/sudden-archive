...
**그룹 D(관리자 통계 대시보드)는 1단계까지 구현 완료됨 — 확정된 내용은 `docs/DECISIONS.md`, 스키마는 `docs/DATABASE.md` 참고:**
- 1단계(클릭수 추적 스키마 + 기록 로직): `item_clicks` 이벤트 로그 테이블, `openOverlay()`에서 `trackClick()`을 fire-and-forget 호출 — 확정, 구현 완료
- 2단계(대시보드 UI·집계 쿼리, 항목 생명주기 관리 등): 아직 아이디어 단계, 세부 목록은 `docs/TODO.md`의 "2단계: 커뮤니티·운영 기능" 참고
