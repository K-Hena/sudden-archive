...
> Version: 1.4
> Last Updated: 2026-08-04 (그룹 F-5 진입점 정리 완료 반영)
...
- 관리자 로그인 시 헤더에 "Master" 버튼 노출 → Master 대시보드(통계/항목 관리·휴지통/맵 관리/댓글/승인 대기)에서 운영

---

## 구 Admin (삭제 완료)

기존 별도 Admin 사이트의 기능은 User 사이트 Master 대시보드가 모두 대체했다. 2026-08-04 Vercel 프로젝트·배포, GitHub `K-Hena/sudden-archive-admin` 저장소와 로컬 복제본을 삭제했으며 두 공개 URL의 404를 확인했다. 현재 운영 진입점은 User 사이트 하나뿐이다.

---

# 시스템 구조

User 사이트의 Master 대시보드
...
3. User/Admin 사이트 통합과 구 Admin 사이트 정리 완료
...
- [x] F-5: Master "컨텐츠 추가" 탭 제거, 컨텐츠 등록 진입점을 홈(`openHomeAdd()`)으로 통일. 구 Admin 기능 격차 재확인 결과 격차 없음
- [x] F-6: "맵 지명" 관리자 전용 등록과 Master 활성 항목/휴지통 이동·원래 상태 복구 구현
- [x] F-4 보완: Master 승인 대기 목록의 맵·태그 조합 필터와 결과 수·빈 상태 구현
- [x] 구 Admin Vercel 프로젝트·GitHub 저장소·로컬 복제본 삭제

이후 진행된 세부 작업 이력(커밋 단위)은 `docs/CHANGELOG.md` 참고.

## 남은 작업

계획이 확정된 구현 항목은 없다. 후속 아이디어는 `docs/TODO.md` 참고.
...
그룹 A~E와 그룹 D-2 1~5단계는 완료됐다. 그룹 F도 홈 대시보드, 사용자 등록 생명주기, Master 승인 기본 흐름, F-5 진입점 정리와 F-6 등록 권한·휴지통까지 완료됐다. 최신 세부 상태는 `docs/TODO.md`를 기준으로 한다.
...
**그룹 F(홈 대시보드 + 사용자 컨텐츠 등록·승인)는 F-6까지 구현 완료됨:**
...
- 일반 사용자 등록·수정·승인 전 숨김, 관리자 승인·반려, 승인 후 관리자 전용 휴지통 이동 정책 구현
- F-5: Master의 중복 `컨텐츠 추가` 탭 제거로 진입점을 홈으로 통일, 구 Admin 기능 격차 재확인(격차 없음)
- F-6: 일반 사용자는 `위폭`/`팁`만 등록하고 `맵 지명`은 관리자만 등록하도록 UI·RLS를 함께 제한. Master 항목 관리에 활성/휴지통 전환과 원래 상태 복구 추가
- F-4 보완: 승인 대기 목록에 맵·태그 조합 필터와 필터 결과 수·조건별 빈 상태 추가. 그룹 F의 계획된 항목은 모두 완료
