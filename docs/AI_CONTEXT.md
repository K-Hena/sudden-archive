...
- 관리자 로그인 시 헤더에 "Master" 버튼 노출 → Master 대시보드(통계/영상 추가/항목 관리/맵 관리)에서 전체 CRUD
...
**※ User 사이트의 Master 대시보드가 CRUD를 대체하면 이 사이트는 정리(폐기)될 예정. 맵/항목 CRUD는 이관 완료(그룹 D-2 4단계), 나머지 기능 격차는 `docs/architecture/admin-flow.md` 참고.**
...
**참고**: 이 체크리스트는 2026-07-18 오전 기준이며, 이후 작업이 여러 건 더 진행되어
"2차: 편집모드 토글" 방식 자체가 그룹 D-2 4단계에서 **Master 대시보드로 완전히 대체**됐다(전역 `editMode` 변수와 카드·맵 타일 호버 액션은 삭제됨). "3차: CRUD 이식" 목표는 형태만 바뀌어 Master의 항목 관리·맵 관리 탭으로 달성 완료됐다.
완료/진행중/예정이 더 세분화된 최신 상태는 `docs/TODO.md`를 참고 — 이 문서보다 최신이다.
Master 대시보드 이식 완료/미완료 항목과 레거시 Admin과의 기능 격차는 `docs/architecture/admin-flow.md` 참고.
...
Claude(Chat)와 사용자가 분류한 작업 중 그룹 A·B·C는 완료됐다. 그룹 D는 1단계(클릭수 추적 스키마·기록 로직)까지 구현이 끝났고 2단계(대시보드 전면 개편)는 아직 아이디어 단계다. 그룹 E는 여전히 착수 전(예정)이며 세부 설계가 확정되지 않았다(`docs/TODO.md`의 "예정"/"아이디어" 섹션 참고). 남은 부분의 구현 방식은 착수 시점에 Claude(Chat)와 별도로 설계한다.
...
**그룹 D(관리자 통계 대시보드) 1단계와 그룹 D-2(Master 대시보드 전면 개편) 1~4단계 모두 구현 완료됨 — 확정된 내용은 `docs/DECISIONS.md`, 스키마는 `docs/DATABASE.md` 참고:**
- 그룹 D 1단계(클릭수 추적 스키마 + 기록 로직): `item_clicks` 이벤트 로그 테이블, `openOverlay()`에서 `trackClick()`을 fire-and-forget 호출 — 확정, 구현 완료
- 그룹 D-2 1~4단계(Master 버튼·셸·통계 탭 / 영상 추가 탭 / 항목 관리 탭 / 맵 관리 탭 + 기존 편집모드 완전 제거): 확정, 구현 완료. 남은 것은 5단계(댓글 탭, 항목 생명주기 관리 등 나머지 아이디어)뿐 — 세부 목록은 `docs/TODO.md`의 "2단계: 커뮤니티·운영 기능" 참고

**아직 미정(그룹 C 이후):**
- 유튜브 채널명 수집 API 방식 — 미정
- 채널 검색을 기존 검색창에 통합할지 별도로 둘지 — 미정
- 그룹 D 2단계(대시보드 전면 개편) 세부 설계 — 아이디어 단계, `docs/TODO.md` 참고
...
- 검색 기능 고도화
- 2단계: 커뮤니티·운영 기능(댓글/문의, 대시보드 전면 개편 등) — 아직 아이디어 단계, 세부 목록은 `docs/TODO.md`의 "2단계" 섹션 참고
