...
## 홈 히어로 재설계 + 하단 3개 섹션 빈 상태 카드화

지시서(`지시서-홈히어로재설계-빈상태카드화.md`)는 Claude(Chat)와 시안으로 확정한 히어로 문구·레이아웃을 반영하고, 위 "UI 개선" 작업에서 만든 `emptyStateHtml()`을 하단 3개 섹션(내가 추가한 컨텐츠/내 즐겨찾기/최근 본 컨텐츠)에 버튼 없는 변형으로 확장하는 작업이었다. Codex 설계 리뷰에서 지시서 문구와 실제 코드가 다른 지점 여러 곳을 발견해 아래처럼 조정했다.

**태그("ARCHIVE") 서체는 원래도 Paperlogy가 아니었음 — 문구 변경만 반영**: 지시서는 "태그: Paperlogy 유지"라고 적었지만, 실제 `.home-kicker`는 처음부터 Paperlogy가 아니라 `Rajdhani`/`Pretendard`였다(`.home-hero h2` 헤드라인만 Paperlogy). "유지"라는 표현의 의도(서체는 바꾸지 않는다)를 그대로 따라, 태그 텍스트만 "TACTICAL ARCHIVE"→"ARCHIVE"로 바꾸고 서체는 손대지 않았다.

**헤드라인 Paperlogy Bold → 기존 800(ExtraBold) 재사용, 700 새로 로드하지 않음**: `styles.css`의 `@font-face`는 Paperlogy 800과 900만 로드하며 700(Bold)은 로드된 적이 없다. 지시서는 "Paperlogy Bold"를 요구하면서 동시에 "기존 Paperlogy 제목 역할 유지"라고도 했는데, 기존 `.home-hero h2`가 이미 font-weight 800을 쓰고 있어 두 표현이 충돌한다. "기존 제목 역할 유지"를 우선해 새 웹폰트 굵기를 추가로 로드하지 않고 기존 800을 그대로 재사용했다 — 새 CDN 요청을 늘리지 않는 쪽이 지시서의 상위 의도(제목 스타일 유지)에 더 맞는다고 판단했다.

**히어로 통계 카드는 헤더 카운터와 같은 시점에 같은 값을 한 번만 계산**: `clipCount`/`tipCount`는 별도 함수가 아니라 `loadAll()` 안의 인라인 로직(`publicItems().filter(...)`)이었다. 새 집계 로직을 만들지 않기 위해, 기존 필터 결과를 지역 변수(`clipCount`, `tipCount`)로 한 번만 계산한 뒤 헤더 엘리먼트(`#clipCount`/`#tipCount`)와 히어로 통계 카드 엘리먼트(`#heroClipCount`/`#heroTipCount`) 양쪽에 같은 값을 대입하도록 `loadAll()`을 최소 수정했다. 두 숫자가 항상 일치하는 이유는 계산이 물리적으로 한 곳뿐이기 때문이다.

**하단 3개 섹션 빈 상태 폭 통일 방식 — `emptyStateHtml()`에 옵션 파라미터 2개 추가**: `emptyStateHtml(icon, headline, desc, buttonLabel, buttonOnclick)`은 검색 결과 없음/승인 대기 없음 등 다른 4곳에서도 쓰이는 공용 함수라, 이 3곳 전용 스타일(240px 고정 폭, 36px 원형 배지, 박스 전체 클릭)을 넣기 위해 기존 호출부를 건드리지 않는 선에서 트레일링 옵션 파라미터 `extraClass`(CSS 클래스 추가)와 `boxOnclick`(박스 전체의 onclick, 지정 시 `empty-state-clickable` 클래스로 커서도 포인터로 바뀜)을 추가했다. 두 파라미터 모두 생략하면 기존 4곳의 동작·마크업은 그대로다. 이 3곳은 새 `home-empty-compact` 클래스로 폭·배지 스타일을 지정하고, 버튼 파라미터(`buttonLabel`/`buttonOnclick`)는 빈 문자열로 넘겨 버튼 자체를 생략한다.

**폭 통일 전 실제 상태 정정**: 지시서는 "내 즐겨찾기는 좁은 폭, 최근 본 컨텐츠는 박스 자체가 없었다"고 적었지만, 실제로는 즐겨찾기와 최근 본 컨텐츠 둘 다 이미 같은 `.home-card-grid`(4열 그리드) 안에서 같은 `emptyStateHtml()`을 렌더링하고 있어 폭이 이미 서로 같았다(그리드 1칸 폭, 반응형이라 고정값은 아니었음). 폭이 실제로 달랐던 것은 "내가 추가한 컨텐츠"(`.submission-list`, 전체 폭) 하나뿐이다. 다만 셋 다 240px 고정폭으로 통일한다는 지시서의 최종 목표 자체는 그대로 반영했다 — 위 서술 오류는 "현재 상태 설명"에만 해당하고 "해야 할 일"에는 영향이 없었다.

**최근 본 컨텐츠는 기존 버튼/이동 기능을 의도적으로 제거**: 지시서는 "최근 본 컨텐츠는 클릭 가능한 목적지가 없다"고 전제했지만, 실제로는 기존에도 "전체 맵 둘러보기" 버튼(`showMapGrid()`)이 있었다. 다만 작업 2와 검증 항목 양쪽에서 "최근 본 컨텐츠는 클릭 무반응인 것이 정상"이라고 반복 명시했으므로, 사실 오류가 아니라 기존 이동 기능을 의도적으로 제거하는 설계 변경으로 판단해 그대로 반영했다(버튼 제거 + `boxOnclick` 미지정으로 박스 클릭도 비활성).

**히어로 배경 장식(`::after` 원형 그래픽) 제거**: `.home-hero::after`가 그리던 붉은 동심원 장식과, 모바일 브레이크포인트의 위치 보정 규칙을 함께 삭제했다. 장식이 차지하던 우측 공간은 새 통계 카드 2개 + "전체 보기" 버튼이 대신한다.

---

## 컨텐츠 추가 모달: 안내 문구 개선 + 좌우 배치 + 제목 서체

지시서(`지시서-컨텐츠추가모달-안내문구개선.md`)는 컨텐츠 추가 모달 첫 단계(붙여넣기/파일 선택)의 힌트 문구를 구체화하고, 데스크톱 좌우 배치를 확인하고, 모달 제목에 기존 Paperlogy를 적용하는 작업이었다. Codex 설계 리뷰로 지시서의 전제 하나가 실제 코드와 다름을 확인했다.

**데스크톱 좌우 배치는 이미 구현되어 있었음 — CSS 변경 없음**: 지시서는 "현재 두 박스가 세로로 쌓이는 원인이 되는 CSS를 확인해 데스크톱에서 좌우로 배치한다"고 전제했지만, 실제로는 `.paste-choices{display:flex;gap:12px;}`(기본값 `flex-direction:row`)가 데스크톱 기본 스타일이고, `@media(max-width:600px){.paste-choices{flex-direction:column;}}`에서만 세로로 전환된다 — 즉 데스크톱은 원래부터 좌우 배치였고 모바일(600px 이하, 390px 포함)만 세로였다. Codex 리뷰와 Playwright 스크린샷(1280px/390px)으로 직접 재확인했다. 이미 있는 동작을 다시 구현하면 불필요한 중복이므로 레이아웃 CSS는 건드리지 않고, 새 힌트 문구가 좁은 박스 안에서 자연스럽게 줄바꿈되는지만 검증했다(데스크톱 2줄, 모바일 2줄로 줄바꿈되지만 겹침·잘림 없음).

**힌트 문구는 디바이스 분기 없이 하나의 문구로 통합, 단 지시서 확정 문구는 커밋 전 리뷰에서 사실과 다름이 드러나 수정**: `navigator.userAgent`나 터치 지원 여부로 문구를 분기하는 것도 가능했지만, 지시서가 "과도한 분기 로직 지양"을 명시했고 텍스트 안에 기기별 방법을 모두 담는 쪽이 분기 코드 없이 같은 정보를 전달할 수 있어 더 단순했다는 원칙은 그대로 따랐다. 다만 지시서가 확정한 문구 "유튜브 링크 또는 이미지 복사 후 Ctrl+V (모바일은 길게 눌러 붙여넣기)"는 커밋 전 Codex 리뷰에서 사실과 다르다는 지적을 받았다 — `.paste-box`는 `<button>`이라 모바일에서 길게 눌러도 네이티브 붙여넣기 메뉴가 뜨지 않고, 실제로는 버튼을 누르면 `readAddClipboard()`가 `navigator.clipboard.read()`로 클립보드를 자동으로 읽어온다(`app.js`). 그 자동 읽기가 실패했을 때만 실제로 붙여넣기 가능한 `#pasteFallback` textarea가 나타나며, 거기서만 길게 눌러 붙여넣기가 의미 있다. "모바일 사용자에게 Ctrl+V가 아닌 실행 가능한 안내를 준다"는 지시서의 상위 의도는 지키되, 실제 동작과 일치하도록 문구를 "유튜브 링크 또는 이미지 복사 후 눌러주세요 (자동으로 가져와요)"로 바꿨다 — 데스크톱 클릭·모바일 탭 모두 같은 자동 읽기 경로를 타므로 기기 분기 없는 단일 문구 원칙도 그대로 유지된다.
