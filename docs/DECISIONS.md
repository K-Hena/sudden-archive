...
## 클립 구간 지정: 버튼(레거시 이식) + 슬라이더(신규)를 같은 변수로 동기화, 슬라이더는 겹치지 않고 위아래 배치

**결정**: 영상 클립 구간(`clip_start`/`clip_end`) 지정 UI를 레거시 Admin의 버튼 방식(`markClipStart`/`markClipEnd`/`clearClip`)과 신규 슬라이더 방식(`<input type="range">` 2개, 시작/끝) 두 가지로 제공하되, 둘 다 동일한 전역 변수 `clipStart`/`clipEnd`를 읽고 쓰도록 해서 항상 같은 상태를 공유한다(값을 두 군데에 따로 들고 동기화 코드로 맞추는 대신, 애초에 단일 소스로 둠). 슬라이더 2개는 하나의 트랙 위에 겹쳐 그리는 대신 위아래로 나란히 배치했다.

**슬라이더를 겹치지 않고 나란히 배치한 이유**: 듀얼 핸들(겹친 트랙 위 손잡이 2개) UI는 새 라이브러리 없이 순수 `<input type=range>`만으로 구현하려면 두 range를 같은 트랙 영역에 절대 위치로 겹치고 클릭 우선순위(어느 핸들을 잡을지) 문제를 직접 풀어야 해서 버그 여지가 크다. 이번 작업 범위(새 라이브러리 없이 네이티브 HTML/JS만 사용)에서는 각자 독립된 range 2개를 위아래로 두는 편이 네이티브 동작(드래그, 키보드 방향키 접근성)을 그대로 활용할 수 있고 구현·검증이 단순해, 이 방식을 택했다.

**시작/끝 상호 보정**: 시작 슬라이더가 끝 슬라이더 값을 넘어가면 끝을 "시작+1초"로, 끝이 시작 이하가 되면 시작을 "끝-1초"로 밀어서 항상 시작 < 끝을 유지한다(레거시 버튼 방식의 "끝은 시작보다 뒤여야 해요" 에러 처리와 동일한 불변식을 슬라이더에도 적용).
