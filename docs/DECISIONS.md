...
## "맵 지명" 항목 이미지 업로드: 크롭 없이 원본 그대로 저장

**결정**: 레거시 Admin 사이트는 이미지 업로드 시 Cropper.js(CDN)로 자른 뒤 업로드하지만, User 사이트 편집모드에는 Cropper.js가 로드되어 있지 않다. 사용자 승인 없이 새 라이브러리를 추가하지 않는다는 원칙에 따라, 이번 이식에서는 Cropper.js를 새로 도입하지 않고 파일 선택 또는 Ctrl+V로 받은 이미지를 크롭 없이 그대로 Storage에 업로드하는 최소 구현으로 진행했다.

**이유**: "새로운 라이브러리는 반드시 검토 후 추가한다"(AI_CONTEXT.md 개발 원칙)는 원칙과, 이번 작업 지시서에서 "크롭 기능 도입 여부는 별도로 보고"하도록 명시한 지시를 따름. 크롭 UI 도입 여부는 사용자와 별도로 논의해서 결정해야 한다.

**남은 영향**: 사용자가 세로로 매우 길거나 큰 원본 이미지를 그대로 붙여넣으면 카드 썸네일에서 비율이 다르게 보일 수 있다(카드 썸네일은 `background-size:cover`로 잘라서 보여주므로 레이아웃이 깨지지는 않는다). 크롭이 필요해지면 Cropper.js 도입 여부를 먼저 결정한 뒤 진행한다.
